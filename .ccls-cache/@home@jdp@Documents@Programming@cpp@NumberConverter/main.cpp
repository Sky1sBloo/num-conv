...
void baseToDecimal(long double value, int base)
{
	...
	long int intValue = std::floor(value);
	int intAns = 0;
	std::cout << value << "   " << intValue << std::endl;
	for (int digit = 0; digit < std::floor(std::log10(intValue)) + 1; digit++)
	{
		int digitValue = static_cast<long int>(std::floor(intValue / std::pow(10, digit))) % 10;  // Returns the digit from intValue
		int digitAns = digitValue * static_cast<int>(std::pow(base, digit));

		intAns += digitAns;
		std::cout << digitValue << " * " << base << "^" << digit << " = " << 
			digitAns << std::endl;
	}
